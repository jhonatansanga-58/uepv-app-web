...
interface NotificationDisableModalProps {
  id: number | null;
  title: string;
  active: boolean;
  open: boolean;
...
export default function NotificationDisableModal({
  id,
  title,
  active,
  open,
  onClose,
  onUpdated,
}: NotificationDisableModalProps) {
  const handleConfirm = async () => {
    if (!id) return;

    const res = await fetch(`/api/notices/notifications/${id}`, {
      method: "PATCH",
