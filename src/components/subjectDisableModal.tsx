...
export default function SubjectDisableModal({
  ...
  const handleDisable = async () => {
    ...
    try {
      ...
      toast.success(active ? `¡La materia "${name}" ha sido deshabilitada con éxito!` : `¡La materia "${name}" ha sido habilitada con éxito!`);
      onUpdated(); // refresh list
      onClose(); // close modal
    } catch (err) {
      const errMsg = err instanceof Error ? err.message : "Error al cambiar estado de la materia";
      toast.error(errMsg);
    } finally {
      setLoading(false);
    }
